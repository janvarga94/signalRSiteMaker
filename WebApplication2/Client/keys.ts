...
export module Keyboard {
    ...
    export var Keyboard = new function () {  //keyboard object
    ...
    document.onkeydown = function (e) {
        ...
        if (Keyboard.keyToBool["DELETE"]) {
            if (GlobalVars.selected.el != document.body) {
                window['myHubProxy'].invoke("removeElement", GlobalVars.selected.el.id);
                var parent = GlobalVars.selected.el.parentElement;
                parent.removeChild(GlobalVars.selected.el);
                Functions.changeSelection(parent);

            }
        }
        if (Keyboard.keyToBool["INSERT"]) {
            if (Editor.editor.style.visibility == 'visible')
                Editor.editor.style.visibility = 'hidden';	// Insert button means show or hide options
            else Editor.editor.style.visibility = 'visible';
        }
        if (Keyboard.keyToBool['CONTROL'] && Keyboard.keyToBool['C']) {				//copy
            if (GlobalVars.selected.el != document.body)
                GlobalVars.Clipboard = GlobalVars.selected.el.outerHTML;
        }
        ...
        if (Keyboard.keyToBool['CONTROL'] && Keyboard.keyToBool['V']) {				// paste
            if (GlobalVars.Clipboard != null) {
                var div = document.createElement('div');
                div.innerHTML = GlobalVars.Clipboard;
                var elementToPaste = div.childNodes[0];
                elementToPaste['id'] = Functions.generateUniqueId();
                Functions.attachHandlersToNewCreatedElement(elementToPaste);
                GlobalVars.selected.el.appendChild(elementToPaste);
                var selectedId = GlobalVars.selected.el.nodeName.toLowerCase() == "body" ? "" : GlobalVars.selected.el.id;
                window['myHubProxy'].invoke("addElement", Functions.getOterHtmlForElement(elementToPaste), selectedId);
            }
        }
        if (Keyboard.keyToBool['SHIFT'] && Keyboard.keyToBool['S']) {			//save in some way
            Functions.exportProject('yourProject.html');
        }
    }
